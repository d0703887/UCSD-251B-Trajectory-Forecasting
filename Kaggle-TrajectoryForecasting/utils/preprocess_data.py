...
class ArgoverseSocialAttnSampling(Dataset):
    ...
    def __getitem__(self, idx):
        cur_data = self.data[idx, :, :, :5]  # (A, T, 5)

        is_vehicle = (self.data[idx, :, 0, 5] == 0) & ~((self.data[idx, :, 0, 0] == 0) & (self.data[idx, :, 0, 1] == 0))  # (A, )
...
#     tmp = DataLoader(ArgoverseSocialAttnSampling(), shuffle=True)
